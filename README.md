...
//To initialize the object of class 'places'
...
void initialise()
{
    ...
    //initializing the global 'temp' 2-D array
    ...
    for (int i = 0; i < MAXN; i++)
    {
        for (int j = 0; j < MAXN; j++)
        {
            if (graph[i][j] == INF)
            {
                //-1 Implement that the places/vertices are not connected
                temp[i][j] = -1;
            }
            else
            {
    ...
    
      {
        ...
        
        {
            ...
            
            {
                ...
                //if no path is present in between
...

{
    ...
    //return an empty array if no path present
...
void path_finder()
{
    ...
    //taking source and destination from the user
...
//to convert an entered string into integer values for further use
